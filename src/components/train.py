...
import archs
from src.losses.margin_losses import *
from scheduler import *

gpus = tf.config.experimental.list_physical_devices('GPU')
if gpus:
  try:
    # Configura para que a alocação de memória cresça conforme a necessidade
    for gpu in gpus:
...
def main():
    ...
    callbacks = [
        ModelCheckpoint(os.path.join('models', args.name, 'model.keras'), verbose=1, save_best_only=True),
    ...
    model.load_weights(os.path.join('models/%s/model.keras' %args.name))
    if 'face' in args.arch:
